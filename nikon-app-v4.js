...
function update(){
  ...
  $('lensInfoText').textContent=$('lensName').value||'Lens';
  $('lensNoText').textContent=$('lensNumber').value||'1';
  if(single){
    const b=+$('beforeSingle').value||0, a=+$('singleValue').value||0;
    $('singleOutput').textContent=signed(a);
    $('wideMenuRow').innerHTML=nikonRow('AF',a,false);
    $('teleMenuRow').classList.add('hidden');
    ...
    $('resultLine').textContent=`Single AF Fine-Tune ${signed(b)} → ${signed(a)}. Saved as lens No. ${$('lensNumber').value}.`;
    ...
    $('wideMenuRow').innerHTML=nikonRow('WIDE',w,false);
    $('teleMenuRow').classList.remove('hidden');
    $('teleMenuRow').innerHTML=nikonRow('TELE',t,true);
    ...
    $('resultLine').textContent=`Wide ${signed(bw)} → ${signed(w)}. Tele ${signed(bt)} → ${signed(t)}. Saved as lens No. ${$('lensNumber').value}.`;
...
function printReport(){
  ...
  const lensNo=$('lensNumber').value||'1';
  ...
  const menuHTML=$('nikonMenu').outerHTML.replace('id="nikonMenu"','');
